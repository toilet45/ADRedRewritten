...
export const AutoGlyphProcessor = {
  ...
  filterValue(glyph) {
    ...
    switch (this.scoreMode) {
      ...
      case AUTO_GLYPH_SCORE.LOWEST_ALCHEMY: {
        if (GlyphInfo[glyph.type].hasAlchemyResource !== true) return new Decimal(0);
  ...
  getRidOfGlyph(glyph) {
    ...
    if (glyph.type === "amalgam" || glyph.type === "musical") {
...
export function getGlyphLevelInputs() {
  ...
  scaledLevel = logScaleSoftcap(scaledLevel, staticFactors.hyperSuperInstability,
    new Decimal(75).div(MendingUpgrade(4).effectOrDefault(1)).add(ExpansionUpgrade(5).effectOrDefault(0)));
