 ...
 * @member {CelestialStudyState[]} selectedStudies   Array of all given valid studies, whether or not they are actually
 *  accessible or purchasable in the given order
 * @member {CelestialStudyState[]} purchasedStudies  Array of studies which were actually purchased, using the given amount
...
export class CelestialStudyTree {
  ...
  constructor(studies) {
    ...
    switch (typeof studies) {
      case "string":
        ...
        if (CelestialStudyTree.isValidImportString(studies)) {
      ...
      case "undefined":
        ...
        break;
      default:
        throw new Error("Unrecognized input parameter for CelestialStudyTree constructor");
    }
  }
  ...
  static isValidImportString(input) {
    ...
    CelestialStudyTree.sets.forEach((_, x) => test = test.replaceAll(new RegExp(`${x},?`, "gu"), ""));
    return /^,?((\d{2,3}(-\d{2,3})?)\b,?)*(\|\d{1,2}!?)?$/iu.test(test);
  }

  // Getter for all the studies in the current game state
  static get currentStudies() {
    const currentStudies = player.celestialstudy.studies.map(s => CelestialStudy(s));
    if (player.challenge.eternity.unlocked !== 0) {
      currentStudies.push(CelestialStudy.eternityChallenge(player.challenge.eternity.unlocked));
    }
  ...
  static commitToGameState(studyArray, auto = true, startEC = false) {
    for (const item of studyArray) {
      const study = typeof item === "number" ? CelestialStudy(item) : item;
      ...
      if (startEC && study instanceof ECCelestialStudyState) EternityChallenge(study.id).start(auto);
    }
    GameCache.currentCelStudyTree.invalidate();
  }
...
*/
  ...
  parseStudyImport(input) {
      ...
      for (const studyRange of studyCluster) {
        ...
        for (const study of studyArray) {
          if (studyDB.includes(parseInt(study, 10))) {
            const tsObject = CelestialStudy(study);
    ...
    if (ecID !== 0) output.push(CelestialStudy.eternityChallenge(ecID));
    return output;
  }
  ...
  checkCelestialStudyNumber(token) {
    const tsNumber = parseFloat(token);
    if (!CelestialStudy(tsNumber)) {
      return 0;
    }
    return tsNumber;
  }
  ...
  attemptBuyArray(studyArray, checkCosts) {
    for (const study of studyArray) {
      const toBuy = typeof study === "object" ? study : CelestialStudy(study);
  ...
  hasRequirements(study, checkOnlyStructure = false) {
    ...
    const check = req => (typeof req === "number"
      ? this.purchasedStudies.includes(CelestialStudy(req))
      : req());
    ...
    if (study instanceof ECTimeStudyState) {
      if (this.purchasedStudies.some(s => s instanceof ECCelestialStudyState)) return false;
      const hasForbiddenStudies = false;
      ...
      const totalCT = player.celestialstudy.theorem;
      const hasEnoughCT = totalCT.subtract(this.spentTheorems).gte(study.cost);
      const secondaryGoal = study.isEntryGoalMet; //Perk.studyECRequirement.isBought || study.isEntryGoalMet;
      return reqSatisfied && !hasForbiddenStudies && (study.isBought || (secondaryGoal && hasEnoughCT));
    }
    return reqSatisfied;
  }
  ...
  buySingleStudy(study, checkCosts) {
    ...
    if (checkCosts) {
      const maxCT = Currency.celestialTheorems.value.add(GameCache.currentCelStudyTree.value.spentTheorems);
      const hasCT = this.spentTheorems.add(config.cost.gte(maxCT));
      if (!hasCT) return;
    }

    // Don't add the costs nor add the study if it is one using ST and there are none
    this.spentTheorems = this.spentTheorems.add(config.cost);

    this.purchasedStudies.push(study);
  }

  get currDimPathCount() {
    return [71, 72, 73, 74].countWhere(x => this.purchasedStudies.includes(CelestialStudy(x)));
  }
...
/* NYI
  get dimensionPaths() {
    ...
    for (const path of validPaths) {
      const pathEntry = CelestialTimeStudies.pathList.find(p => p.path === path);
  ...
  get ec() {
    ...
    const ecStudies = this.purchasedStudies.find(s => s instanceof ECCelestialStudyState);
  ...
  get exportString() {
    return `${this.purchasedStudies
      .filter(s => s instanceof NormalCelestialStudyState)
      .map(s => s.id)
