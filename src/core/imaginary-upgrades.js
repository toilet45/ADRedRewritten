...
class ImaginaryUpgradeState extends BitPurchasableMechanicState {
  ...
  tryUnlock() {
    ...
    if (MendingUpgrade(17).boughtAmount.lt(7) && !Laitela.isDamaged) GameUI.notify.reality(`You've unlocked an Imaginary Upgrade: ${this.config.name}`);
  ...
  onPurchased() {
    ...
    if (this.id === 22) {
      for (const key of Object.keys(player.reality.glyphs.sac)) {
        if (key !== "amalgam" && player.reality.glyphs.sac[key].lt(ImaginaryUpgrade(22).effectValue)) player.reality.glyphs.sac[key] = ImaginaryUpgrade(22).effectValue;
      }
    }
    if (this.id === 25) {
...
ImaginaryUpgradeState.index = mapGameData(
  ...
  config => (config.id <= 10 || config.id >= 26
