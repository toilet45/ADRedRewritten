...
export const MachineHandler = {
  ...
  get uncappedRM() {
    ...
    if (!PlayerProgress.realityUnlocked() && !Achievement(191).isUnlocked) {
    ...
    if (rmGain.gte(1) && rmGain.lt(10)) rmGain = (log10FinalEP).mul(27).div(4000).minus(26);
  ...
  get currentIMCap() {
    return player.reality.iMCap.times(ImaginaryUpgrade(13).effectOrDefault(1)).times(PerkShopUpgrade.imCap.effectOrDefault(1));
  },

  // This is iM cap based on in-game values at that instant, may be lower than the actual cap
  get projectedIMCap() {
    return this.baseIMCap.times(ImaginaryUpgrade(13).effectOrDefault(1)).times(PerkShopUpgrade.imCap.effectOrDefault(1));
  },
  ...
  gainedImaginaryMachines(diff) {
    if (MendingUpgrade(3).boughtAmount.gte(4) && !Laitela.isDamaged) return this.currentIMCap.sub(Currency.imaginaryMachines.value);
