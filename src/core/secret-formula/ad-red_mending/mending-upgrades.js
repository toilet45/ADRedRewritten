...
const rebuyable = props => {
  props.cost = () => getHybridCostScaling(
    player.mending.mendingRebuyables[props.id],
    DC.E30,
    props.initialCost,
    props.costMult,
  ...
  if (props.effect) props.effect = p => effect(p ?? player.mending.mendingRebuyables[props.id]);
  if (props.effects) props.effects = p => effects(p ?? player.mending.mendingRebuyables[props.id]);
...
const hybridRebuyable = props => {
  const purAmnt = () => Math.min(player.mending.mendingHybrids[props.id].toNumber(), props.purchaseLimit);
