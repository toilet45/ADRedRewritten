 ...
 * @abstract
 */
class TierTwoAlchemyResourceState extends GameMechanicState {
  constructor(config) {
    super(config);
    this.ema = new ExponentialMovingAverage(0.01, 10, 100, 0.01);
    this._before = 0;
  ...
  get data() {
    return player.celestials.ra.alchemyV2[this.id];
  }
  ...
  get reaction() {
    return AdvancedAlchemyReactions.all[this.id];
  }
...
class TierTwoBasicAlchemyResourceState extends TierTwoAlchemyResourceState {
  constructor(config) {
    super(config);
    // The names are capitalized, so we need to convert them to lower case
    // in order to access highestRefinementValue values which are not capitalized.
  ...
  get highestRefinementValue() {
    return player.celestials.ra.highestV2RefinementValue[this._name];
  }

  set highestRefinementValue(value) {
    player.celestials.ra.highestV2RefinementValue[this._name] = Decimal.max(this.highestRefinementValue, value);
  }

  get cap() {
    return Decimal.clampMax(Ra.advancedAchemyResourceCap, this.highestRefinementValue);
  }
}

class TierTwoAdvancedAlchemyResourceState extends TierTwoAlchemyResourceState {
  get cap() {
    return this.reaction.reagents.map(x => x.resource.cap).reduce((a, b) => Decimal.min(a, b));
  }
}
...
export const AdvancedAlchemyResource = mapGameDataToObject(
  GameDatabase.celestials.alchemyV2.resources,
  config => (config.isBaseResource
    ? new TierTwoBasicAlchemyResourceState(config)
    : new TierTwoAdvancedAlchemyResourceState(config))
);

export const AlchemyResources = {
  all: AlchemyResource.all,
  base: AlchemyResource.all.filter(r => r.isBaseResource),
  allAdvanced: AdvancedAlchemyResource.all,
  baseAdvanced: AdvancedAlchemyResource.all.filter(r => r.isBaseResource),
};
...
export const AdvancedAlchemyReactions = (function() {
  // For convenience and readability, stuff is named differently in GameDatabase
  function mapReagents(resource) {
    return resource.config.reagents
      .map(r => ({
        resource: AlchemyResources.allAdvanced.find(x => x.id === r.resource),
        cost: r.amount
      }));
  }
  return {
    all: AlchemyResources.allAdvanced
      .map(r => (r.isBaseResource ? null : new AdvancedAlchemyReaction(r, mapReagents(r))))
  };
}());
