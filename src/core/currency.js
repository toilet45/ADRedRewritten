...
MathOperations.decimal = new class DecimalMathOperations extends MathOperations {
  ...
  subtract(left, right) { return left.gt("ee9") ? left : Decimal.subtract(left, right).max(0); }
...
Currency.eternities = new class extends DecimalCurrency {
  ...
  get startingValue() {
    ...
    if (Enslaved.isExpanded) return new Decimal(ExpansionUpgrade(10).canBeApplied ? 1 : 0);
    return Effects.max(
      0,
      RealityUpgrade(10)
    );
...
Currency.eternityPoints = new class extends DecimalCurrency {
  ...
  get startingValue() {
    ...
    if (Enslaved.isExpanded) return new Decimal(ExpansionUpgrade(10).canBeApplied ? 6 : 0);
    return Effects.max(
      0,
      Perk.startEP1,
      Perk.startEP2,
...
Currency.darkMatter = new class extends DecimalCurrency {
  ...
  set value(value) {
    const capped = Decimal.min(value, Laitela.isDamaged ? DC.BEMAX : Number.MAX_VALUE);
