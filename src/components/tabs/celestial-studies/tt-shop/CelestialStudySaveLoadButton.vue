...
export default {
  name: "CelestialStudySaveLoadButton",
  components: {
    HoverMenu,
  },
  props: {
  ...
  methods: {
    ...
    load() {
      ...
      if (this.preset.studies) {
        ...
        const combinedTree = new CelestialStudyTree();
        combinedTree.attemptBuyArray(CelestialStudyTree.currentStudies, false);
        combinedTree.attemptBuyArray(combinedTree.parseStudyImport(this.preset.studies), true);
        CelestialStudyTree.commitToGameState(combinedTree.purchasedStudies, false, combinedTree.startEC);

        const presetName = this.name ? `Study preset "${this.name}"` : "Study preset";
        GameUI.notify.eternity(`${presetName} loaded from slot ${this.saveslot}`);
      } else {
        Modal.message.show("This Celestial Study list currently contains no Celestial Studies.");
      }
    },
    respecAndLoad() {
      if (Player.canEternity) {
        player.respec = true;
        const newTree = new CelestialStudyTree();
        newTree.attemptBuyArray(newTree.parseStudyImport(this.preset.studies));
        animateAndEternity(() => CelestialStudyTree.commitToGameState(newTree.purchasedStudies, false, newTree.startEC));
      }
    },
    deletePreset() {
      this.hideContextMenu();
      if (this.preset.studies) Modal.studyString.show({ id: this.saveslot - 1, deleting: true });
      else Modal.message.show("This Celestial Study list currently contains no Celestial Studies.");
    },
    handleExport() {
      this.hideContextMenu();
